...
1. Run the installed extension workflow creator for bugfix in JSON mode and parse its output for BUG_ID, BRANCH_NAME, and BUG_REPORT_FILE. All file paths must be absolute.
  **IMPORTANT** You must only ever run this script once. The JSON is provided in the terminal as output - always refer to it to get the actual content you're looking for.

2. Load `extensions/workflows/bugfix/bug-report-template.md` from the extension package to understand required sections.
...
4. Load the bugfix tasks template from the extension package and create `tasks.md` in the bug directory with concrete task descriptions based on the bug.
